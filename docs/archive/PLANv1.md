# StackMind — Runtime Flaw Analysis & Improvement Plan (v1)

**Purpose:** This document analyzed five agent handoff reports against the StackMind protocol spec (D021-D031) and identified 22 flaws with fixes. It was the foundation for the v1.x runtime governance improvements.
...
- **Date:** 2026-06-18
- **Updated:** 2026-06-19 (v1.1.0/v1.2.0 verification)
- **Status:** COMPLETED — all critical/high items addressed in v1.2.0+
...
All identified flaws and their fixes are documented below, grouped by scope.

## 3. v1.1.0/v1.2.0 Verification Status (Part 0)

The `stackmind` package (the engine repo, separate from this analysis) was checked against this plan. This verification is based on externally documented behavior.

### Addressed (13 of 22 items)

| ID | Fix | Evidence |
|---|---|---|
| PLAT-01 | Boot integrity anchors to `INDEX.yaml` | Layer 4: "Canonical drift — TREE.yaml work-order totals must match INDEX.yaml" |
| PLAT-03 | Write-lock via `stackmind lock acquire/release/status` | `.sync/runtime/LOCK`; `shutdown` is the canonical release mechanism |
| PLAT-04 | Normalization writes a `NORMALIZATION` decision entry | `stackmind promote` records it in `decisions/` on success |
| PLAT-05 | `.sync-ref` anchoring added | Layer 4: "live .sync HEAD must match the .sync-ref SHA tracked by the main repo" |
...
| GEMMA-02 | Inbox drain gate blocks shutdown | `stackmind shutdown`: "must contain zero unprocessed items" |
...
| LOCAL-LLM-02 | Canonical writes through validated promote | Validation gates surround both sides of the write |

### Partial (3 items)

| ID | What shipped | What's still missing |
|---|---|---|
| PLAT-02 | `graph_version` field added | No formal broadcast mechanism |
| GEMMA-04 | Inbox drain gate helps | No deferred-item pathway |
| LOCAL-LLM-03 | Stale-version commit risk reduced | No post-commit readback |

### Still Open (6 items)

- **CLAUDE-02**: No dispatch-confirmation tracking
- **CLAUDE-03**: Session numbering notation unstandardized
- **GEMMA-03**: Quality metrics lack commit hash/timestamp
- **GEMINI-01**: Broken local test environment has no formal blocker protocol
- **GEMINI-02**: No tooling check against worker self-assignment
- **LOCAL-LLM-01**: No schema field forcing delegated-vs-initiated action disclosure

### New issues found in v1.1.0/v1.2.0

1. `tree_version` vs `graph_version` naming — RESOLVED (two distinct fields by design)
2. Version drift between docs — RESOLVED (genuine migration path)
3. Write-lock is advisory only — needs CLI-only writes in `AGENTS.md`
4. Inbox drain gate has no deferred-item pathway
5. No stated test coverage for engine validators
6. Schema drift evidence — `normalize_enum_field` migration needed

## 4. Platform-Level Flaws (Part 1)
...
## 6. Priority Matrix (Part 3)

| Priority | ID | Flaw | Agents affected | Status |
|---|---|---|---|---|
| Critical | PLAT-01 | Boot integrity validates stale vs stale | All | ✅ Addressed |
| Critical | GEMMA-01 | Snapshot at tree_version: 1 — continuity broken | Gemma | ✅ Addressed |
| High | PLAT-02 | `tree_version` used as broadcast signal | All | 🟡 Partial |
| High | PLAT-03 | No concurrency guard on `.sync` mutations | Claude, Codex, Local-LLM | ✅ Addressed |
| High | CLAUDE-01 | Draft promotion without schema validation | Claude | ✅ Addressed |
| High | CODEX-01 | Draft snapshot written with stale tree_version | Codex | ✅ Addressed |
| High | CODEX-02 | Untracked `reviews/.` treated as a note | Codex | ✅ Addressed |
| High | GEMMA-02 | 10 review requests silently dismissed | Gemma | ✅ Addressed |
| High | GEMINI-01 | Local test failure mis-classified as non-blocking | Gemini | ⬜ Open |
| High | GEMINI-02 | WO-056 self-assigned by worker | Gemini | ⬜ Open |
| High | LOCAL-LLM-01 | Overclaims Claude's work as its own | Local-LLM | ⬜ Open |
| High | LOCAL-LLM-02 | Committed without `stackmind validate` | Local-LLM | ✅ Addressed |
| Medium | PLAT-04 | Normalization actions bypass decision log | Claude | ✅ Addressed |
| Medium | PLAT-05 | `.sync` git-ignore creates audit blindness | All | ✅ Addressed |
| Medium | CLAUDE-02 | Inbox directives unconfirmed before handoff | Claude | ⬜ Open |
| Medium | CODEX-03 | No cross-validation against INDEX.yaml | Codex | ✅ Addressed |
| Medium | GEMMA-03 | Quality metrics without commit hash | Gemma | ⬜ Open |
| Medium | GEMMA-04 | No awareness of Gemini's incoming review request | Gemma | 🟡 Partial |
| Medium | GEMINI-03 | Two WOs bundled in one review request | Gemini, Gemma | ✅ Addressed |
| Medium | GEMINI-04 | Post-release changes without declared release target | Gemini | ✅ Addressed |
| Medium | LOCAL-LLM-03 | Committed tree_version unconfirmed | Local-LLM | 🟡 Partial |
| Low | CLAUDE-03 | Session numbering notation ambiguous | Claude | ⬜ Open |

## 7. Recommended Implementation Order (Part 4)
...
**Phase 2 — Add write-ordering (PLAT-03)** ✅ Shipped as `stackmind lock`. Note: the lock is advisory-only — `AGENTS.md` should mandate CLI-only writes.

**Phase 3 — Fix draft write timing (CODEX-01 + all workers)** ✅ Shipped — shutdown re-reads TREE at write time.

**Phase 4 — Per-agent protocol compliance** Partially shipped (GEMMA-02, GEMINI-03, GEMINI-04, LOCAL-LLM-02 done). Still open: GEMINI-01, GEMINI-02, LOCAL-LLM-01, GEMMA-03, CLAUDE-02, CLAUDE-03.

**Phase 5 — Audit trail completeness (PLAT-04, PLAT-05)** ✅ Both shipped.

**Phase 6 — (added 2026-06-19)**
1. Resolve the `tree_version`/`graph_version` naming ambiguity.
2. Add the deferred-receipt pathway to the shutdown inbox-drain gate.
3. Add a `LOCK_STOLEN` compliance event for `--force` lock acquisition.
4. Publish a coverage threshold for `validators/`.
5. Address the remaining behavioral items via `AGENTS.md` contract updates.

## 8. Resolution in AGENTS.md

The 6 open behavioral items (CLAUDE-02, CLAUDE-03, GEMMA-03, GEMINI-01, GEMINI-02, LOCAL-LLM-01) were resolved by adding contract rules to `AGENTS.md`:

- **PLAT-03**: CLI-only writes; lock theft logs compliance event
- **GEMINI-01**: Broken local test env → BLOCKED + BUGFIX WO
- **GEMINI-02**: Workers cannot self-assign WOs
- **LOCAL-LLM-01**: Handoff must distinguish delegated vs initiated
- **GEMMA-03**: Quality metrics require commit SHA, branch, tested_at
- **CLAUDE-02**: Messages section renamed with `unread_inbox_count`
