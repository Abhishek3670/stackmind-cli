...
The implementation order must begin with the verification and trust foundation. Autonomous procedural learning should not be enabled until the underlying verification substrate can substantiate the trust claims made by the learning system.
...
PHASE 1 — EXPERIENCE CAPTURE
        ↓
PHASE 2 — EXPERIENCE COMPILER
        ↓
PHASE 3 — PATTERN MINING
        ↓
PHASE 4 — SKILL DISTILLATION
        ↓
PHASE 5 — REPLAY + VERIFICATION
        ↓
PHASE 6 — BASELINE / PERFORMANCE EVALUATION
        ↓
PHASE 7 — RISK-BASED PROMOTION + VERSIONING
        ↓
PHASE 8 — CONTINUOUS REFINEMENT + DECAY
```

The learning architecture remains future-compatible with richer event sources such as a TUI, structured user corrections, and event-sourced runtime telemetry, but those are not prerequisites for the initial learning implementation.
...
# 28. Phase 0 — Verification & Trust Foundation

Phase 0 is a **prerequisite workstream**, not a small preamble. It establishes the trust substrate that every later learning phase depends on.

> **The learning system is only as trustworthy as the verification substrate underneath it.**

If the Harness cannot independently establish what changed and whether the resulting implementation is correct, a later learning system could promote procedures labeled "verified" without sufficient evidence. That would amplify trust debt rather than reduce it.

## 28.1 Existing Trust and Security Debt

Before autonomous learning is enabled, resolve known weaknesses in the current trust boundary, including:

```text
write-lock / TOCTOU race
path-traversal exposure
weak secret redaction
prompt-injection surface
```

A learning system amplifies weaknesses in verification because a single incorrect outcome can become persistent reusable knowledge.
...
## 28.2 Runner-Owned Actual Change Detection

The authoritative change detector should be owned by the runtime/runner, not by the LLM.

The selected mechanism is:

> **Capture a runner-owned before/after workspace snapshot, derive the actual filesystem change set, and use patch/diff output plus semantic analysis as verification layers.**

The LLM-reported `modified_files` or any future `diff` field may remain useful as a declaration, but it is not authoritative.
...
Text / patch diff
  ↓
LibCST / AST semantic analysis
  ↓
Behavioral verification
```

At minimum, the snapshot should capture:
...
size
content hash
permission / executable metadata where relevant
```

The detector must include untracked/new files in addition to tracked-file changes; plain `git diff` already captures changes and deletions to tracked files, but it does not by itself provide complete coverage of untracked/new files, so it is insufficient as the canonical detector.

A full-tree before/after snapshot introduces non-trivial I/O and hashing overhead on every Harness run, and therefore adds to the existing runtime cost of workspace copying. This is an intentional correctness tradeoff for Phase 0 rather than a claim of zero-cost verification. The implementation should measure the overhead and prefer incremental/path-scoped snapshotting, metadata-first checks, and content hashing only when required while preserving complete detection of added, modified, and deleted files.

## 28.3 Observed Changes vs. Agent Declarations

The runner should compare the agent's declaration against the independently observed change set.

If the observed set differs from the agent declaration, the run should not be treated as a routine success. It should be blocked or routed to the existing **Gemma / QA-lead review path**, which can decide whether the mismatch is expected, requires correction, or constitutes a verification failure.
...
The runner-observed state is authoritative. A file omitted from the LLM declaration but modified on disk must still be subject to scope, policy, and verification checks.

## 28.4 Enforcement Against the Observed Change

The contract boundary should move from:
...
The LLM declaration becomes supplementary evidence rather than the basis for authorization.

## 28.5 Verification Layers

Verification should become a layered system:

```text
LEVEL 1 — Filesystem delta
    What physically changed?
             ↓
LEVEL 2 — Text / patch diff
    What content changed?
             ↓
LEVEL 3 — LibCST / AST analysis
    What program structure changed?
             ↓
LEVEL 4 — Tests / contracts / behavioral checks
    Did the resulting system remain correct?
```

The existing LibCST-based knowledge compiler should therefore be reused as a **semantic enrichment layer**, not as the primary mechanism for discovering all filesystem changes.

## 28.6 Explicit Verification Dimensions

Avoid a single overloaded boolean such as:

```text
verified = true
```

Instead, expose explicit dimensions:
...
Each dimension should have its own evidence source and status. A partially checked artifact must not be represented as completely verified.

## 28.7 Learning Eligibility Gate
...
An execution that cannot be adequately verified remains observable history but cannot automatically become positive learning evidence.
...
│               │                │
↓               ↓                ↓
Learning      History only     Failure analysis
eligible
```

This is the canonical definition of the trust boundary for self-learning and supersedes any separate restatement of the same idea elsewhere in the document.

## 28.8 Prompt-Injection and Historical-Content Boundary

Historical agent output, reports, messages, and experience records must not become trusted instructions merely because they are persisted.
...
The latter requires explicit transformation, policy checks, and verification.

## 28.9 Definition of Done for Phase 0

Phase 0 is complete only when all of the following are true:

```text
✓ Known concurrency / TOCTOU risk addressed
✓ Path-safety controls hardened
✓ Secret handling / redaction strengthened
✓ Prompt-injection boundary defined and enforced
✓ Runner-owned before/after change detection implemented
✓ Untracked additions and deletions captured
✓ Observed-vs-declared change comparison implemented
✓ Scope enforcement operates on observed changes
✓ Patch / diff artifact generated independently of LLM claims
✓ LibCST / AST semantic analysis integrated where applicable
✓ Behavioral verification covers relevant tests / contracts
✓ Verification dimensions are explicit
✓ Learning-eligibility gate is enforced
```

> **No autonomous skill promotion is permitted until Phase 0 is complete.**

---

# 29. Three-Level Trust Model

The canonical trust model is the same progression defined in §28.7:
...
This section exists only to make the implementation dependency explicit; §28.7 is the authoritative definition of the gate.

---

# 30. Implementation Boundary

The design is now conceptually complete enough to hand to implementation planning. The next work should focus on concrete schemas, interfaces, test fixtures, and incremental changes rather than introducing additional conceptual layers without evidence that they are necessary.

The first implementation target is therefore not self-learning. It is the **verification substrate that makes self-learning safe to enable**.
...
The final architectural thesis remains:

> **StackMind should increase its capabilities from experience without allowing its learned knowledge to outrun the evidence supporting its trust.**
