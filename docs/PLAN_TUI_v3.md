...
# 5. Phase P6-0 — Repository Validation

## Goal

Freeze implementation assumptions against the actual branch.
...
- daemon startup path;
- TUI startup path;
- current TUI source tree;
- current RPC methods;
- session creation/resume behavior;
- actual runtime operation entry point;
- policy/contract enforcement path;
- tool execution path;
- event persistence;
- restart recovery;
...
Produce a small implementation map documenting:
...
- Every implementation task below maps to a real module.
- No task relies on an unverified placeholder.
- Existing tests pass before modification.
